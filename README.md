...
| `AI_LLM_TIMEOUT_MS` / `AI_LLM_MAX_RETRIES` / `AI_EMBEDDING_TIMEOUT_MS` / `AI_EMBEDDING_MAX_RETRIES` | provider 요청 timeout/재시도 상한 (Phase 13 - `executeWithResilience()`가 모든 실제 provider 호출에 적용) |
| `AI_PROVIDER_FAILOVER_ENABLED` / `AI_SECONDARY_LLM_PROVIDER` / `AI_SECONDARY_LLM_MODEL` / `AI_SECONDARY_LLM_API_KEY` | 2차 LLM provider fallback (Phase 13 §18 - narrow error set만 대상, 실제 사용된 provider가 usage 기록에 반영됨) |
...
| `AI_SHADOW_MODE` / `AI_SHADOW_SAMPLE_RATE` / `AI_SHADOW_LLM_PROVIDER` | Shadow mode (Phase 13 §20, **기본값 OFF**) - 샘플링된 요청만 비동기로 2차 provider와 비교, 사용자에게는 절대 노출되지 않고 구조화된 품질 지표만 기록 |
...
- Phase 10C: `docker-compose.yml`에 `migrate`/`app`(`smoke` profile) + `redis`/`minio`(기본 실행)를 추가했습니다 — `docker compose --profile smoke run --rm migrate` / `up -d app` / `pnpm smoke:test`. (Phase 14 Part 1) 위와 함께 실제 build/run까지 검증 완료. `.github/workflows/`에 PR CI/Docker build+push/staging 배포/야간 실인프라 테스트 workflow를 추가했지만, 이 세션에는 GitHub remote가 연결되어 있지 않아 실제 GitHub-hosted runner에서 실행해 본 적은 없습니다.
