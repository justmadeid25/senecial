...
2. `next start`(production build) 자체가 이 Turbopack dev-server 특유의 문제를 겪지 않는지 별도 검증(`e2e-real-infra` 경로에서 확인 가능)해야 합니다.
...
- Selector 안정화는 부분적으로만 수행했습니다 - 141건의 `getByText(` 중 `.first()` 등으로 스코프된 것은 32건뿐이라는 사전 조사 결과가 있었으나, 전체 재작성은 아직 진행하지 못했습니다.
- Worker 완료 대기는 사전 조사에서 고정 `waitForTimeout` 사용이 0건으로 확인되어 이미 양호했습니다 - 추가 전용 polling 유틸리티는 만들지 않았습니다.
- DB 관측 lightweight diagnostic은 구현하지 않았습니다.
- 실패 artifact redaction 자동 검증은 별도로 구현하지 않았습니다 - 기존 Playwright trace(`retain-on-failure`)가 요청 헤더의 Authorization 등을 자체적으로 어떻게 다루는지 별도로 감사하지 못했습니다.
