...
`release.yml`은 **"production 준비(readiness gate)"** 워크플로입니다 - 실제 production 환경으로의 배포 자동화는 이 저장소의 범위 밖입니다(실제 Kubernetes/Terraform 등 production 인프라 자체는 구현하지 않습니다). `release.yml`이 실제로 하는 일:
...
이 문서가 명시하는 대로, 다음은 아직 구현되지 않았습니다:
